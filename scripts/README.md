...
48 个文件（不含本索引），分六节。**通用**的三机可用；**机器专属**的只在对应机器有意义
（保留是因为它们记录了各机的真实配置与历史事故，合并只会互相污染）。

命名里的机器代号：`5090` = 2×RTX5090 本地机，`6000a` = 4×RTX6000Ada 本地机，
`runpods` = 4×RTX5090 云机。

> 📁 **这些脚本各自跑在什么机器上、依赖哪些 env / 权重 / 补丁,见
> [`doc/machines/`](../doc/machines/)** —— 三份机器档案的第 6 节列了每个脚本注入的运行期
> 环境变量与 serve flags,第 7 节是从零复现步骤。改机器专属脚本前先对一遍那里的实测值。
...
> ⚠️ 这个 210s 是**从 switch 到能正常出片**的整段墙钟；`doc/machines/runpods-5090x4.md`
> 记的 **175–180s 是到 `/health` 返回 200**。两个数不冲突也不可互换 ——
> **`/health` 早于权重加载就绪**，所以切换后的第一单必然还是 warmup，不计时。
